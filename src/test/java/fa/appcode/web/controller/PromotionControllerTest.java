...
class PromotionControllerTest {
	...
	void testSave4() throws Exception {
		...
		mockMvc.perform(MockMvcRequestBuilders.multipart("/admin/promotion/save").file("file", file.getBytes())
				.param("promotionId", promotionId.toString()).param("title", title).param("startTime", "2021-12-01")
				.param("endTime", "2021-12-01").param("discountLevel", String.valueOf(discountLevel))
				.param("detail", detail)).andExpect(MockMvcResultMatchers.status().isOk())
	...
	void testSave5() throws Exception {
		...
		mockMvc.perform(MockMvcRequestBuilders.multipart("/admin/promotion/save").file(file)
				.param("promotionId", promotionId.toString()).param("title", title).param("startTime", "2021-12-01")
				.param("endTime", "2021-12-01").param("discountLevel", String.valueOf(discountLevel))
				.param("detail", detail).param("image", "/resources/img/promotion/Untitled2.png"))
	...
	void testSave6() throws Exception {
		...
		mockMvc.perform(MockMvcRequestBuilders.multipart("/admin/promotion/save").file(file)
				.param("promotionId", promotionId.toString()).param("title", title).param("startTime", "2021-12-01")
				.param("endTime", "2021-12-01").param("discountLevel", String.valueOf(discountLevel))
				.param("detail", detail)).andExpect(MockMvcResultMatchers.status().isBadRequest())
