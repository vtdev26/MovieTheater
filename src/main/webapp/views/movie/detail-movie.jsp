...
<div>
    ...
    <form action="#" method="post" name="formMovie" id="formMovie" enctype="multipart/form-data">
        ...
        <div class="form-group">
            ...
            <input type="text" class="form-control" id="movieNameEnglish" name="movieNameEnglish"
        ...
        <div class="form-group">
            <label for="movieProductCompany" class="font-weight-bold">Production Company <span
                    class="text-danger">(*)</span></label>
            <input type="text" class="form-control" id="movieProductCompany" name="movieProductCompany"
            ...
            <div class="custom-control custom-checkbox">
                <div class="row">
                    <c:forEach items="${types}" var="type">
                        <div class="col-sm-4">
                            <div>
                                <input type="checkbox" class="custom-control-input" name="types" id="${type.typeName}"
                                       value="${type.typeId}" ${typeIdss.contains(type.typeId) ? 'checked': ''}> <label
                                    class="custom-control-label" for="${type.typeName}">${type.typeName}</label>
                            </div>
                        </div>
                    </c:forEach>
            ...
            <div class="custom-control custom-checkbox">
                <div class="row">
                    <c:forEach items="${schedules}" var="schedule">
                        <div class="col-sm-4">
                            <div>
                                <input type="checkbox" class="custom-control-input" name="schedules"
                                       id="${schedule.scheduleId}"
                                       value="${schedule.scheduleId}" ${scheduleIdss.contains(schedule.scheduleId) ? 'checked': ''}>
                                <label class="custom-control-label"
                                       for="${schedule.scheduleId}">${schedule.scheduleTime}</label>
                            </div>
                        </div>
                    </c:forEach>
