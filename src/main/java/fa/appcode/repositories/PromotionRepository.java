...
public interface PromotionRepository extends JpaRepository<Promotion, Integer> {
	...
	@Query("SELECT p FROM Promotion p WHERE p.detail LIKE :searchData "
			+ "OR p.title LIKE :searchData "
			+ "OR p.discountLevel LIKE :searchData "
			+ "OR function('convert',varchar, p.endTime, 103) LIKE :searchData "
			+ "OR function('convert',varchar, p.startTime, 103) LIKE :searchData")
